            ...
            {purchasedCourses.map((course) => (
              <GlassPanel key={course.id} hoverEffect className="flex flex-col gap-4 overflow-hidden p-0 border border-border/30">
                <div className="relative aspect-video w-full overflow-hidden bg-muted/30 group">
                  <div className="absolute inset-0 bg-linear-to-tr from-primary/10 via-primary/5 to-transparent dark:from-primary/20 dark:via-purple-500/10 dark:to-transparent mix-blend-multiply dark:mix-blend-normal transition-opacity group-hover:opacity-75" />
                  {course.image && (
                    <img 
                      src={course.image}
                      alt={course.title}
